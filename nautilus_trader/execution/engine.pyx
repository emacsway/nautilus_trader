...
# -- EVENT HANDLERS -------------------------------------------------------------------------------
    ...
    cpdef void _determine_position_id(self, OrderFilled fill, OmsType oms_type):
        ...
        if position_id is not None:
            ...
            if self.debug:
                self._log.debug(f"Assigned {position_id!r} to {fill}.")
            return
        ...
        if primary.position_id is None:
            ...
            self._log.debug(f"Assigned primary order {position_id!r}.")
    ...
    cpdef PositionId _determine_hedging_position_id(self, OrderFilled fill):
        ...
        if order.exec_spawn_id is not None:
            ...
            for spawned_order in exec_spawn_orders:
                if spawned_order.position_id is not None:
                    if self.debug:
                        self._log.debug(f"Found spawned {spawned_order.position_id!r} for {fill}.")
        ...
        if self.debug:
            self._log.debug(f"Generated {position_id!r} for {fill}.")
        return position_id
